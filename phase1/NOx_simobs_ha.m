...
NO2 = reshape(NO2,98,74,24,nDays); % O3(lon,lat,hour,day)
...
for i=1:nsite
    ...
    if in==1
        ...
        NO2_obs(site_count,:,:)=chemdata(:,:,i,8);
...
for i=1:(site_count-1)  % here site_count-1=144
    for j=1:24
        for k=1:nDays
            NO2_obs(i,j,k)=NO2_obs(i,j,k+80); % O3_obs(site,hour,day)
        end
    end
end
...
for i=1:nDays
    for j=1:6
        ...
        [~,h]=m_contourf(lon,lat,NO2(:,:,hour(j)+1,i).*gd,0:10:30); % Note here for data range
        set(h,'edgecolor','none');
        hold on
        mbmin=0;
        mbmax=30; % Note here for data range
        pdata=NO2_obs(:,hour(j)+1,i).*gd1(:,1)*22.4/48;
        ...
        caxis([0,30]); % Note here for data range
        % color=WhiteBlueGreenYellowRed(10);
        color=colormap(jet);
        colormap(color(:,:));
        hc=colorbar('ytick',0:5:30,'fontsize',15); % Note here for data range
        ...
        title(['NO_2 ','2021-03-',num2str(i+22-1),'-',num2str(hour(j)+8)],'fontsize',17);

        Now_printing_figure_on=['2021-03-',num2str(i+22-1),'-',num2str(hour(j)+8)]
        path = ['D:/files/Master/02学术/Case/Shaog03/figure_MO/NO2_h/NO2_','2021-03-',num2str(i+22-1),'-',num2str(hour(j)+8)]; % Note here to modify
