...
for i=1:26
        lat_use(:,i)=squeeze(lat(54,:));
        % lat_use(:,i)=squeeze(lat(46,:));
end
...
% O3 = squeeze(nanmean(O3(45:47,:,:,:),1));
% O3 = reshape(O3(:,1:26,73:144),74,26,24,3); % O3(lat,layer,hour,day)
NOx = ncread(cmaqFile1,'NOX'); % NOx(lon,lat,layer,tstep)
nDays = size(NOx,4)/24; % get the length of tstep to calculate days number
NOx = squeeze(nanmean(NOx(45:47,:,:,:),1));
NOx = reshape(NOx(:,1:26,73:144),74,26,24,3); % NOx(lat,layer,hour,day)
...
% i=2;j=8;
        ...
        contourf(lat_use,squeeze(height(54,:,:,j,i)),NOx(:,:,j,i),'edgecolor','none');
        caxis([0,30]);
        ...
        quiver(lat_use(1:3:74,8:2:20),squeeze(height(54,1:3:74,8:2:20,j,i)),...
        squeeze(vwind(54,1:3:74,8:2:20,j,i))/25,squeeze(wwind(54,1:3:74,8:2:20,j,i))/3,...
        ...
        xlim([20 25.4]);
        ylim([0 2]);
        set(gca,'Xtick',[20:1:25.4]);
        set(gca,'Ytick',[0:0.4:2],'Yticklabel',{'0','400','800','1200','1600','2000'});
        xlabel('Latitude(°N)','FontSize',12);
        ylabel('Height(m)','FontSize',12);
        grid off

        if (j>=1)&&(j<=16) 
            hour=[8:23];
            title(['NOx 2021-03-',num2str(i+25-1),'-',num2str(hour(j))],'fontsize',12,'FontWeight','bold');
            Now_printing_figure_on=['2021-03-',num2str(i+25-1),'-',num2str(hour(j))]
            path = ['F:/Figure/caseSG3/fig_add/NO2_yz/NOx_','2021-03-',num2str(i+25-1),'-',num2str(hour(j))]; % Note here to modify
            print(gcf,'-dtiff','-r600',path)
        else
            hour=[0:7];
            title(['NOx 2021-03-',num2str(i+25),'-',num2str(hour(j-16))],'fontsize',12,'FontWeight','bold');
            Now_printing_figure_on=['2021-03-',num2str(i+25),'-',num2str(hour(j-16))]
            path = ['F:/Figure/caseSG3/fig_add/NO2_yz/NOx_','2021-03-',num2str(i+25),'-',num2str(hour(j-16))]; % Note here to modify
            print(gcf,'-dtiff','-r600',path)
        end
    end
end
